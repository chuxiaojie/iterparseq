...
from .model_abinet_iter import ABINetIterModel as Model, IterNetModel
...
class ABINet(CrossEntropySystem):
    ...
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        self.model = Model(max_label_length, self.eos_id, self.num_classes, iter_size, d_model, nhead, d_inner,
                           ...
                           l_loss_weight, l_num_layers, l_detach, l_use_self_attn, a_loss_weight, img_size=kwargs['img_size'])
...
class IterNet(CrossEntropySystem):

    def __init__(self, num_iters: int, share_weights: bool, deep_supervision: bool, 
                 charset_train: str, charset_test: str, max_label_length: int,
                 batch_size: int, lr: float, warmup_pct: float, weight_decay: float,
                 iter_size: int, d_model: int, nhead: int, d_inner: int, dropout: float, activation: str,
                 v_loss_weight: float, v_attention: str, v_attention_mode: str, v_backbone: str, v_num_layers: int,
                 l_loss_weight: float, l_num_layers: int, l_detach: bool, l_use_self_attn: bool,
                 l_lr: float, a_loss_weight: float, lm_only: bool = False, resnet_kwargs: dict=dict(), **kwargs) -> None:
        super().__init__(charset_train, charset_test, batch_size, lr, warmup_pct, weight_decay)
        self.scheduler = None
        self.save_hyperparameters()
        self.max_label_length = max_label_length
        self.num_classes = len(self.tokenizer) - 2  # We don't predict <bos> nor <pad>
        self.model = IterNetModel(max_label_length, self.eos_id, self.num_classes, iter_size, d_model, nhead, d_inner,
                           dropout, activation, v_loss_weight, v_attention, v_attention_mode, v_backbone, v_num_layers,
                           l_loss_weight, l_num_layers, l_detach, l_use_self_attn, a_loss_weight, img_size=kwargs['img_size'],
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # Pretrain submodels independently first
            ...
            # We also train the alignment model to 'satisfy' DDP requirements (all parameters should be used).
            # We'll reset its parameters prior to joint training.
            a_res = self.model.alignment(l_res['feature'].detach(), v_res[0]['feature'].detach())
            loss = self.calc_loss(targets, v_res, l_res, a_res)
        else:
            # Reset alignment model's parameters once prior to full model training.
            if self._reset_alignment:
...
class IterVM(CrossEntropySystem):

    def __init__(self, num_iters: int, share_weights: bool, deep_supervision: bool, charset_train: str, charset_test: str, max_label_length: int,
                 batch_size: int, lr: float, warmup_pct: float, weight_decay: float,
                 d_model: int, nhead: int, d_inner: int, dropout: float, activation: str,
                 v_loss_weight: float, v_attention: str, v_attention_mode: str, v_backbone: str, v_num_layers: int,
                 img_size: tuple, resnet_kwargs: dict=dict(), **kwargs) -> None:
        super().__init__(charset_train, charset_test, batch_size, lr, warmup_pct, weight_decay)
        self.scheduler = None
        self.save_hyperparameters()
        self.max_label_length = max_label_length
        self.num_classes = len(self.tokenizer) - 2  # We don't predict <bos> nor <pad>
        self.model = BaseIterVision(max_label_length, self.eos_id, self.num_classes, 
                           v_attention, v_attention_mode, v_loss_weight, 
                           d_model, nhead, d_inner, dropout, activation, v_backbone, v_num_layers,
                           h=img_size[0]//4, w=img_size[1]//4, 
                           num_iters=num_iters, share_weights=share_weights, deep_supervision=deep_supervision, resnet_kwargs=resnet_kwargs)
        self.model.apply(init_weights)
     
    def forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        max_length = self.max_label_length if max_length is None else min(max_length, self.max_label_length)
        logits = self.model.forward(images)['logits']
        return logits[:, :max_length + 1]  # truncate
    ...
    def calc_loss(self, targets, *res_lists) -> Tensor:
        ...
        for i, res_list in enumerate(res_lists):
            ...
            for res in res_list:
                ...
                loss += F.cross_entropy(logits, targets.flatten(), ignore_index=self.pad_id)
            loss /= len(res_list)
            self.log(f'loss_{i}_' + res_list[0]['name'], loss)
            total_loss += res_list[0]['loss_weight'] * loss
        return total_loss
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        images, labels = batch
        inputs, lengths, targets = self._prepare_inputs_and_targets(labels)
        v_res = self.model(images)
        loss = self.calc_loss(targets, v_res)
        self.log('loss', loss)
        return loss