...
from .modules import DecoderLayer, Decoder, Encoder, TokenEmbedding, IterEncoder
...
class PARSeq(CrossEntropySystem):
    ...
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
                 ...
                 decode_ar: bool, refine_iters: int, dropout: float, embed_kwargs: dict, **kwargs: Any) -> None:
        ...
        self.encoder = Encoder(img_size, patch_size, embed_dim=embed_dim, depth=enc_depth, num_heads=enc_num_heads,
                               mlp_ratio=enc_mlp_ratio, embed_kwargs=embed_kwargs)
    ...
    def no_weight_decay(self):
        ...
        all_param_names = self.state_dict().keys()
        for name in param_names:
            if name.endswith('.cls_token') or name.endswith('.dist_token'):
                continue
            if name not in all_param_names: # for debug
                print(name)
...
class IterPARSeq(CrossEntropySystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int, batch_size: int, lr: float, warmup_pct: float, weight_decay: float, img_size: Sequence[int], patch_size: Sequence[int], embed_dim: int, enc_num_heads: int, enc_mlp_ratio: int, enc_depth: int, dec_num_heads: int, dec_mlp_ratio: int, dec_depth: int, perm_num: int, perm_forward: bool, perm_mirrored: bool, decode_ar: bool, refine_iters: int, dropout: float, embed_kwargs: dict, enc_num_iters: int, enc_shared_connection: bool, shared_encoder: bool, num_test_iters: int=None, num_extra_feats: int=None, **kwargs: Any) -> None:
        super().__init__(charset_train, charset_test, batch_size, lr, warmup_pct, weight_decay)
        self.save_hyperparameters()
        ...
        self.encoder = IterEncoder(num_iters=enc_num_iters, shared_connection=enc_shared_connection, shared_encoder=shared_encoder, num_test_iters=num_test_iters,  num_extra_feats=num_extra_feats,
                               img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, depth=enc_depth, num_heads=enc_num_heads,
                               mlp_ratio=enc_mlp_ratio, embed_kwargs=embed_kwargs)
        decoder_layer = DecoderLayer(embed_dim, dec_num_heads, embed_dim * dec_mlp_ratio, dropout)
        self.decoder = Decoder(decoder_layer, num_layers=dec_depth, norm=nn.LayerNorm(embed_dim))
        ...
        # Perm/attn mask stuff
        self.rng = np.random.default_rng()
        self.rng0 = np.random.default_rng(0)
        self.max_gen_perms = perm_num // 2 if perm_mirrored else perm_num
        self.perm_forward = perm_forward
        self.perm_mirrored = perm_mirrored
    ...
    def encode(self, img: torch.Tensor, is_train=False):
        if is_train:
            patch_sizes = self.encoder.sample_patch_size(rng=self.rng0)
        else:
            patch_sizes = None
        outs = self.encoder(img, patch_sizes=patch_sizes)
        if is_train:    # when training
            return outs # outputs of all iterations
        else:   # during inference
            return outs[-1] # outputs from the last iteration
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # Encode the source sequence (i.e. the image codes)
        memories = self.encode(images, is_train=True)
